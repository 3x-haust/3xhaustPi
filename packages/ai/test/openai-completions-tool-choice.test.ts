...
describe("openai-completions tool_choice", () => {
	...
	it("maps groq qwen3 reasoning levels to none reasoning_effort", async () => {
	...
	it("omits tool_stream for unsupported z.ai models", async () => {
		const catalogModel = getModel("zai", "glm-4.7")!;
		const model = { ...catalogModel, compat: { ...catalogModel.compat, zaiToolStream: false } };
		const tools: Tool[] = [
			{
				name: "ping",
				description: "Ping tool",
	...
	it("sends max_tokens for OpenCode completions models", async () => {
		...
		for (const model of cases) {
			...
			expect(compat && "maxTokensField" in compat ? compat.maxTokensField : undefined).toBe("max_tokens");
